...
// TMsgEventType is the golang structure for table t_msg_event_type.
type TMsgEventType struct {
	Id          int    `json:"id"          orm:"id"          description:"自增主键，唯一标识记录"`               // 自增主键，唯一标识记录
	EventType   string `json:"eventType"   orm:"event_type"  description:"事件类型编码(长度限制50字符)，不可为空"`     // 事件类型编码(长度限制50字符)，不可为空
	Alias       string `json:"alias"       orm:"alias"       description:"事件类型别名(长度限制100字符)，默认空字符串"`  // 事件类型别名(长度限制100字符)，默认空字符串
	Description string `json:"description" orm:"description" description:"事件类型详细描述，默认空文本"`            // 事件类型详细描述，默认空文本
